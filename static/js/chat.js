  ...
  $.post(chatServer + '/api/chat/user-connect',
    ...
    function (remoteAddr, status, xhr) {
      ...
      stompClient.connect({username: userName}, function () {
        ...
        stompClient.subscribe('/client/queue/messages', function (output) {
...
function createTextNode(messageObj) {
  ...
  if (!self){
    messageNotification(htmlMessage);
  }
...
/**
 * WS Store update test
 * @param storeObject
 */
function createStoreMessage(storeObject) {
  return '<div>From Server Store Update: ' + storeObject.name + '</div>';
}
